...
def configure_hive():
    try:
        ...
        stm = Hive(
            ...
            keys=[
                os.getenv("HIVE_ENGINE_ACTIVE_PRIVATE_KEY"),
                os.getenv("HIVE_ENGINE_POSTING_PRIVATE_KEY"),
            ],
        )
...
def configure_hive_engine_wallet(account, stm):
    try:
        ...
        print(
            f"{Fore.GREEN}[Success]{Style.RESET_ALL} Hive Engine Wallet configured successfully."
        )
        return wallet
    except Exception as e:
        logging.error(f"Error configuring Hive Engine Wallet: {e}")
        print(
            f"{Fore.RED}[Error]{Style.RESET_ALL} Error configuring Hive Engine Wallet: {e}"
        )
        return None
    ...
    try:
        ...
        for transaction in transactions:
            if transaction.get("memo") == memo:
                ...
                print(
                    f"{Fore.YELLOW}[Warning]{Style.RESET_ALL} Memo already exists in transaction history: {memo}"
                )
                return True
        return False
    except Exception as e:
        ...
        print(
            f"{Fore.RED}[Error]{Style.RESET_ALL} Error checking for existing memo: {e}"
        )
        return False
...
def process_payments(df):
    try:
        ...
        if not payments_enabled:
            ...
            print(
                f"{Fore.CYAN}[Info]{Style.RESET_ALL} Payments are deactivated. Only spreadsheets will be generated."
            )
            return
                ...
                if (
                    ...
                    if delegator not in ignore_payment_accounts:
                        ...
                        unique_hash = generate_unique_hash(
                            delegator, payment_amount, current_date
                        )
                        memo = format_memo(
                            delegator, payment_amount, current_date, unique_hash
                        )
                        if not memo_exists(payment_account.name, memo, token_name):
                            logging.info(f"Processing reward for {delegator}...")
                            print(
                                f"{Fore.CYAN}[Info]{Style.RESET_ALL} Processing reward for {Fore.BLUE}{delegator}{Style.RESET_ALL}..."
                            )
                            if process_payment_for_delegator(
                                wallet,
                                payment_account,
                                token_name,
                                delegator,
                            ...
                            print(
                                f"{Fore.GREEN}[Success]{Style.RESET_ALL} Reward processed for {Fore.BLUE}{delegator}{Style.RESET_ALL}."
                            )
                        else:
                            logging.warning(
                                f"Reward already processed for {delegator}. Skipping reward."
                            )
                            print(
                                f"{Fore.YELLOW}[Warning]{Style.RESET_ALL} Reward already processed for {Fore.BLUE}{delegator}{Style.RESET_ALL}. Skipping reward."
                            )
            except Exception as e:
                logging.error(f"Error making reward to {delegator}: {e}")
                print(
                    f"{Fore.RED}[Error]{Style.RESET_ALL} Error making reward to {Fore.BLUE}{delegator}{Style.RESET_ALL}: {e}"
                )

        if not payments_made:
            logging.info(
                "No rewards were made because there were no eligible delegators or sufficient balances."
            )
            print(
                f"{Fore.CYAN}[Info]{Style.RESET_ALL} No rewards were made because there were no eligible delegators or sufficient balances."
            )

        final_balance = check_balance(wallet)
        logging.info(f"Updated {token_name} balance after rewards: {final_balance}")
        print(
            f"{Fore.GREEN}[Success]{Style.RESET_ALL} Updated {Fore.YELLOW}{token_name}{Style.RESET_ALL} balance after rewards: {Fore.YELLOW}{final_balance}"
        )
    except Exception as e:
