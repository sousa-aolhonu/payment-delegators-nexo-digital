...
def get_account_info(account_name):
    try:
        ...
        logging.info(f"Fetching account info for {account_name}...")
        hive = Hive()
        ...
        logging.info(f"Fetched account info for {account_name}: {own_hp} HP.")
        return own_hp
...
def vests_to_hp(vesting_shares, delegator):
    try:
        ...
        logging.info(f"Converting vests to HP for {delegator}...")
        hive = Hive()
        ...
        logging.info(f"Converted vests to HP for {delegator}: {hp} HP.")
        return hp
    except Exception as e:
